      ...
      <div className={`p-4 sm:p-6 rounded-3xl border transition-all ${
        ...
        <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2.5">
            ...
            {/* Year Dropdown Selector (Menggunakan model dropdown pill) */}
            <div className="relative">
              <select
                value={selectedYear}
                onChange={(e) => setSelectedYear(e.target.value)}
                aria-label="Pilih Periode Tahun"
                className={`text-xs font-black px-3.5 py-2 pr-8 rounded-xl border appearance-none cursor-pointer focus:outline-hidden focus:ring-2 focus:ring-blue-500 transition-all ${
                  isDarkMode 
                    ? 'bg-slate-800 border-slate-700 text-white hover:bg-slate-700' 
                    : 'bg-white border-slate-300 text-slate-900 shadow-xs hover:border-blue-400'
                }`}
              >
                {['2024', '2025', '2026', '2027', '2028', '2029', '2030'].map(yr => (
                  <option key={yr} value={yr} className={isDarkMode ? 'bg-slate-900 text-white' : 'bg-white text-slate-900'}>
                    Tahun {yr}
                  </option>
                ))}
              </select>
